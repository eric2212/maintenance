...
public final class SingleStarttimerCommand
extends ProxyCommandInfo {
    public SingleStarttimerCommand(MaintenanceProxyPlugin plugin) {
        super(plugin, null);
    }
    ...
    public void execute(SenderInfo sender, String[] args) {
        if (args.length == 2) {
            ...
            MaintenanceRunnableBase runnable = this.plugin.startSingleMaintenanceRunnable(server, duration, true);
            sender.send(this.getMessage("singleStarttimerStarted", "%TIME%", runnable.getTime(), "%SERVER%", server.getName()));
        } else {
            sender.send(this.getHelpMessage());
        }
    }
